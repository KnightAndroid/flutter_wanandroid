...
class HtmlWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return (new _HtmlParser(context,isTitle)).parseFromStr(strippedHtml);
  }
}
...
class _HtmlParser {
  ...
  _HtmlParser(this.context, this.isTitle,{this.appContext: const {}})
  ...
  Widget parseFromStr (String htmlStr) {
    // print('*** parsing html...');
    if(isTitle == 0){
      contentStyle = textTheme.title;
    }else if(isTitle == 1){
      contentStyle = textTheme.caption.merge(
        const TextStyle(color: ResColors.colorBack),
      );
    }else {
      contentStyle = StyleTheme.getInstance(context).getTopicStyle();
    }
